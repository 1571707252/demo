...
func (c *ChatGroup) PushMsg(rid int, msg string, i int8) *model.ChatMsg {
	c.userMutex.RLock()
	u, ok := c.users[rid]
	if ok == false {
		return nil
	...
	m := &Msg{Msg: msg, RId: rid, Time: time.Now(), NickName: u.nickName}
	...
	//redis list 数据结构 右进左出
	jsonMsg, _ := json.Marshal(m)
	redis.Pool.RPush(context.Background(), "chat_world", jsonMsg)

	chatMsg := &model.ChatMsg{RId: rid, Msg: msg, NickName: u.nickName, Type: i, Time: time.Now().Unix()}
	// 消息要给所有频道用户广播出去
